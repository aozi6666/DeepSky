...
20. Preview 插件的设计原理是什么？它是怎么做到实时预览和代码高亮的？highlight.js 是怎么集成到文档展示中的？有没有对高亮主题或样式做定制？
    【回答】在这个仓库里，“Preview”本质对应的是 **Storybook 的预览/文档渲染链路**，而不是另外做了一套自研预览器：docs-site 用 `@storybook/react-vite` 把组件库的 stories/MDX 渲染成一个独立文档站点，再通过 preview 配置去定制文档里的代码块渲染与高亮。

    1) **实时预览怎么来的**  
       `apps/docs-site/.storybook/main.ts` 指定 stories 扫描路径覆盖到 `packages/components/src/**/*.stories.*` 和 `packages/components/src/stories/*.mdx`，Storybook 在 dev 模式下基于 Vite 的 HMR 实时编译这些入口，所以组件源码或 story 变更都会直接反映到画布（Canvas）和文档页（Docs）里。

    2) **highlight.js 怎么接入到 Docs 代码块**  
       关键在 `apps/docs-site/.storybook/preview.ts`：通过 `parameters.docs.components.pre` 覆盖 Docs 渲染 `<pre>` 的方式。它会检查 `<pre>` 的唯一子元素是否为 `<code>`，解析其 `className` 里的 `language-xxx`，提取文本内容后交给自定义的 `HighlightedCodeBlock` 组件渲染。

    3) **为什么不用默认高亮，而是自定义组件**  
       `HighlightedCodeBlock.tsx` 里直接引入 `highlight.js`，在 `useEffect` 里对 `<code>` 调用 `hljs.highlightElement(el)`。同时它会主动移除 `data-highlighted`，避免 highlight.js 因为 DOM 被标记过而跳过“内容更新后的再次高亮”，这对 Storybook 里代码示例随参数/渲染变化的场景更稳。

    4) **高亮主题/样式是否定制**  
       有定制：docs-site 没有直接引 highlight.js 自带主题 CSS，而是在 `.storybook/highlight-theme.css` 写了一套简化的 GitHub 风格配色（`.hljs` 及各种 token class），并在 `preview.ts` 里全局引入该 CSS。这样主题更可控，也避免额外依赖与样式覆盖冲突。
